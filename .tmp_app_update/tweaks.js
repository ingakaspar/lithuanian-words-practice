...
(function () {
  ...
  const defaults = window.TWEAK_DEFAULTS || { mood: "vilnius", voice: "editorial", density: "cozy" };
  let state = { ...defaults };
  ...
  const SCHEMA = [
    {
      ...
      options: [
        { value: "vilnius",  label: "Vilnius",  swatch: ["#f7eedd", "#3f2a14"], stripes: ["#e8b53b", "#1f6d3f", "#b6322c"] },
        { value: "midnight", label: "Midnight", swatch: ["#1d2230", "#e0bf76"] },
        { value: "linen",    label: "Linen",    swatch: ["#f5f1e8", "#22201c"] }
      ]
    },
    {
      key: "voice",
      ...
      options: [
        { value: "editorial",  label: "Editorial",  preview: "Aa", style: "font-family:'Spectral',serif;font-style:italic;font-weight:500;" },
        { value: "modernist",  label: "Modernist",  preview: "Aa", style: "font-family:'Bricolage Grotesque',sans-serif;font-weight:700;letter-spacing:-0.035em;" },
        { value: "antique",    label: "Antique",    preview: "Aa", style: "font-family:'DM Serif Display',serif;font-style:italic;" }
      ]
    },
    {
      key: "density",
  ...
  for (const section of SCHEMA) {
    ...
    for (const opt of section.options) {
      ...
      let inner = "";
      if (opt.stripes) {
        inner += `<span class="sw" style="background:linear-gradient(180deg, ${opt.stripes[0]} 0%, ${opt.stripes[0]} 33.3%, ${opt.stripes[1]} 33.3%, ${opt.stripes[1]} 66.6%, ${opt.stripes[2]} 66.6%, ${opt.stripes[2]} 100%);"></span>`;
      } else if (opt.swatch) {
